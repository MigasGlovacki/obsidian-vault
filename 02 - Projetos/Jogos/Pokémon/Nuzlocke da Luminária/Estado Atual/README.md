...
A antiga pasta de snapshots foi removida na reorganização: eram exportações intermediárias regeneráveis e não faziam parte do estado vivo da run.
...
- O estado vivo deve permanecer concentrado em `estado-atual.md` e `estado-atual.json`. Exportações intermediárias não precisam ser versionadas no vault.
